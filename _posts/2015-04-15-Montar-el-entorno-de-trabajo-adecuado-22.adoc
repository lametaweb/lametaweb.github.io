...
El último componente que vamos añadir a nuestro banco de trabajo de partida va a proporcionarnos un Eclipse vitaminado agilizando bastante nuestro trabajo de desarrollo diario. Se trata de un plugin, que es el mecanismo empleado en Eclipse para suministrar características adicionales al IDE, en particular de las JBoss Tools,  
