...
Vamos primero con el fichero de configuración. El fichero de configuración de la unidad de persitencia es _persistence.xml_. Según la especificación JPA2 (JSR-338) en un fichero WAR, cuando no empaquetemos las clases dentro del mismo en otro fichero JAR, el fichero de configuración deberá ir en la carpeta _WEB-INF/classes/META-INF/_. Por lo tanto y con arreglo a la configuración por defecto de Maven para los ficheros de recursos tendremos que crear el fichero en la carpeta _src\main\resources\META-INF\_. Creamos el fichero con la opción de menú _New > Other... > XML > XML File_.
