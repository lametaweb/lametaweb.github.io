...
Bien, veamos lo más relevante del contenido XML:
...
* Elemento <datasource>: Define un origen de datos identificado por un nombre JNDI del tipo java:jboss/datasources/NOMBRE_DS. Como nombre del pool podemos usar el que queramos. El elemento <connection-url> nos dice que ruta de la base de datos estará en el disco duro en la ruta C:\BD\drones\ y que se creará una base de datos vacía si no existía ya anteriormente.
