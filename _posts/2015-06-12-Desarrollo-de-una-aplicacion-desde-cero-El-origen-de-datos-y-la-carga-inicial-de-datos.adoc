...
* Persistimos primero por ejemplo el objeto Drone, usando el método _persist_ del entity manager. Esto provoca un cambio de estado en el objeto Drone desde new a managed. El objeto está ahora dentro del contexto de persistencia. No está aún en la base de datos. Ahora podemos ya establecer la relación muchos a uno en Trabajo entre Trabajo y Drone y persistir el objeto Trabajo.

* Existe una relación de composición entre Trabajo y PuntoRuta y por tanto cuando persistamos un trabajo debemos persistir los puntos de ruta asociados. Esto se implementa con el atributo cascade de la anotación @OneToMany `@OneToMany (cascade = CascadeType.ALL)`. En este punto toda la jerarquía está dentro del contexto en estado managed. Y nada de momento en la base de datos.

* Finalmente establecemos la relación uno a muchos entre Drone y Trabajo y la persistimos. Aquí no uso persist porque el objeto implicado, Drone, ya está en estado managed.
