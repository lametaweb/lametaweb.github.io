...
* Veamos ahora la relación entre Trabajo y PuntoRuta, si miramos el diagrama de diseño observamos que existe una relación de composición, y por tanto cuando persistamos un trabajo deberemos persistir también los puntos de ruta asociados. Esto se implementa con el atributo cascade de la anotación @OneToMany `@OneToMany (cascade = CascadeType.ALL)`. En este punto toda la jerarquía está dentro del contexto en estado managed. Y nada de momento en la base de datos.

Y esto es todo. Como veis hemos sido capaces de implementar las operaciones de persistencia abstrayéndonod totalmente de los detalles de la base de datos.
