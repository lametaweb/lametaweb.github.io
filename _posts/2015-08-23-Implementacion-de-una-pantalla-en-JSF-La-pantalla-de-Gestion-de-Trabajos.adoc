= Implementación de una pantalla en JSF: La pantalla de Gestión de Trabajos.
La metaweb
...
Es importante señalar que no existe una solución única a la hora de implementar un caso de uso, ni para la interfaz, ni para la manera en que implementemos el backing bean. Deberemos intentar siempre conseguir un resultado que proporcione la mejor experiencia de usuario al tiempo que cumpla todos los requisitos acordados con el cliente. Respecto de esto último el cliente podría por ejemplo pedirnos que desde la pantalla CU de trabajos sea posible acceder a otra pantalla que permita añadir un nuevo Drone.
