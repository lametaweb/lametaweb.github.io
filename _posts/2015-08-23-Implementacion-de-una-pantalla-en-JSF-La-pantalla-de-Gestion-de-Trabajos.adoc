...
Una vez instalado el servidor y antes de iniciarlo debemos añadir un usuario. Para ello tenemos que ejecutar el archivo por lotes `bin\add-user.bat`.
...
NOTE: La etiqueta `viewAction` es nueva en JSF 2.2. Asocia un evento a una página y ofrece más flexibilidad que la etiqueta `event` para acciones de precarga de datos para una página. Comentaros que en la página inicial de ejemplo el uso de esta etiquta no sigue el patrón habitual de uso. Un uso más adecuado podremos verlo en la implementación de las páginas de maestro y de detallo del mantenimiento de la entidad Trabajo.
