...
Ya tenemos nuestro proyecto, ahora toca generar una aplicación a partir de éste.

Primero ejecutamos el comando `mvn install`, desde la carpeta del proyecto (donde está el `pom.xml`). Como ya sabemos con este comando ejecutamos la fase install del ciclo de vida estándar de construcción y todas las anteriores, lo que se traduce en la ejecución de una serie de goals de plugins, que llevan a cabo la construcción y distribución en nuestro repositorio local del artefacto ejecutable en forma de fichero `.jar`, que podemos localizar a través del explorador de Windows en la carpeta `.m2/` dentro de nuestra carpeta de usuario, en una estructura de carpetas que dependerá de las coordenadas que elegimos anteriormente para el proyecto.

El fichero `.jar` es el ejecutable de nuestro proyecto y además de en el repositorio local también se crea en la carpeta target dentro
