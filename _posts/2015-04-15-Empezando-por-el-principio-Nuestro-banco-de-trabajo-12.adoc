...
Una vez en la dirección anterior nos vamos directamente al tercer apartado: Creating a Project. Lo que vamos a hacer es crear usando sólo Maven un proyecto Hola mundo! a partir de una plantilla de proyecto. Se trata de una característica adicional que aun no he comentado, Maven es capaz de generar un proyecto que sirva como punto de partida de nuestra aplicación a partir de un molde o plantilla, que Maven denomina arquetipo. Hay muchos arquetipos, muchos han sido creados por la gente de Maven, otros por otras empresas, y nosotros mismo podemos crear un arquetipo, de hecho las empresas crean arquetipos para disponer de puntos de partida de sus proyectos. Los arquetipos están en los repositorios, que son almaceneces gestionados por Maven, donde existen elementos de otra naturaleza con librerías, nuestros proyectos, etc. A todos estos elementos Maven los denomina artefactos. Bien, como indica en ejercicio de cinco minutos abrimos una ventana de comandos, creamos un directorio, por ejemplo c:\prueba-maven, y nos situamos dentro del mismo. A continuación escribimos: