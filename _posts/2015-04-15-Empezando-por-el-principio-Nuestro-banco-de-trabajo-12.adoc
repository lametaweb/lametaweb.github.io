...
Para arrancar con el blog, en estos primeros posts vamos, en primer lugar, a definir e instalar en nuestra máquina un entorno de trabajo adecuado, lo más parecido posible a lo que nos podamos encontrar hoy en una empresa. Y a continuación montaremos una mini aplicación con capa de presentación, negocio y datos. Lo vamos a hacer partiendo de cero y sin ningún tipo de asistente, de modo que consigamos desmitificar la creación de una aplicación Java EE por capas y ganar así la confianza necesarias para seguir, en sucesivos posts, experimentando con el amplio abanico de tecnologías y herramientas disponibles.
...
Bueno manos a la obra. Empecemos con el entorno de trabajo. Como IDE usaremos Eclipse, por supuesto con Maven como gestor del ciclo de desaroollo. Como servidor JBoss y como base de datos en principio algo ligerito, Java DB, también conocido como Derby, que nos evitará instalar una base de datos en esta primera versión de la aplicación.
