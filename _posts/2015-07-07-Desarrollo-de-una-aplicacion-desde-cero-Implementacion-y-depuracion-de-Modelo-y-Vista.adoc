...
----
	...
	<h:body> 
		...
		<h:form>
			...
			<h:inputText id="horas" value="#{disponibilidadBean.horas}"
				required="true">
			...
			<h:inputText id="minutos" value="#{disponibilidadBean.minutos}"
				required="true">
				<f:validateLength maximum="2" />
...
** <h:inputText...> es un componente de entrada-salida de texto. Para el campo fecha configuro un converter predefinido ya que necesito especificar el formato de entrada-salida y la localización de la hora. Para los dos campos numéricos, como no necesito ningún formato especial no uso un converter NumberConverter. En cambio sí configuramos dos validators predefinidos para limitar el número de cifras y el rango del valor introducido. En todos los casos el componente se enlaza con el backing bean a través del atributo value.
