...
. Restore view: En una initial request se crea un objeto View vacío y se asocia al contexto FacesContext. La View es un objeto que contiene una representación en el lado del servidor del árbol de componentes DOM del lado del cliente/navegador. El objeto FacesContext representa el contexto de servidor que contiene todos los elementos para la gestión del ciclo de vida. Después de la creación de la View se salta a la última fase Render Response. En cambio, en una Postback Request la View ya existe (de la Initial request que la precedió) y simplemente se restaura en el contexto con la View ya montada.
