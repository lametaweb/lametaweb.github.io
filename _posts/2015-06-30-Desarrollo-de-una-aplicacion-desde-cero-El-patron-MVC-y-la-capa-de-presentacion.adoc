...
Con esta exposición de los elementos básicos de JSF desde el punto de vista estático y dinámico he querido daros un background que os permita tomar una base sólida para poder implementar sobre JSF cualquier funcionalidad. Además de lo expuesto JSF aporta muchos otros elementos, que poco a poco iréis conociendo, y que aceleran aún más el desarrollo de una aplicación web empresarial. Como ejemplos podríamos citar por ejemplo los ámbitos para guardar el estado de la interfaz en caso de uso multipantalla, tales como el conversation scope, el Flash, o el más avanzado Flow, las capacidades de localización, la creación de clases Converter, Validator o Listener propias, o la implementación de componentes de usuario a medida. Se me ocurre como ejemplo un componente "reloj analógico" que nos mostrara la hora en una determinada localización.
